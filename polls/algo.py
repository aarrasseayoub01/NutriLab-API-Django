...
def retrieveFood(request):
    ...
    for i in range(num_vars):
        variable_names.append(Food[i]["name"])
    variables = pulp.LpVariable.dicts(
        "", variable_names, lowBound=0, upBound=8)

# Define the objective function
    objective = pulp.lpSum(
        [variables[f"{variable_names[i]}"] for i in range(0, num_vars)])

# Define the constraints using a for loop
    constraints = []
    NutriList = ["Calories", "Protein", "Carbs", "Fat",
                 "Fiber", "Salt", "Sugar"]
    for i in range(0, 7):
        constraint = pulp.lpSum(
            [Food[j][NutriList[i]]*variables[f"{variable_names[i]}"] for j in range(0, num_vars)]) >= request[i]
        constraints.append(constraint)

# Create the problem and solve it
    problem = pulp.LpProblem("My LP Problem", pulp.LpMinimize)
    problem += objective
...
retrieveFood([1000, 1000, 1000, 1000, 1000, 1000, 1000])
